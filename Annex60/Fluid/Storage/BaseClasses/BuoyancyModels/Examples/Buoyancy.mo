within Annex60.Fluid.Storage.BaseClasses.BuoyancyModels.Examples;
model Buoyancy
  extends Modelica.Icons.Example;
  replaceable package Medium = Annex60.Media.Water;
  replaceable Buoyancy1 buoyancyModel(nEle=2) constrainedby PartialBuoyancy
  ...
  Annex60.Fluid.MixingVolumes.MixingVolume vol_TOP(
    ...
    each nPorts=2,
    each V=0.01,
    each T_start=313.15) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={10,58})));
  Annex60.Fluid.MixingVolumes.MixingVolume vol_BOTTOM(
    redeclare package Medium = Medium,
    each m_flow_nominal=1,
    each nPorts=2,
    each V=0.01,
    each T_start=293.15) annotation (Placement(transformation(
        ...
        rotation=-90,
        origin={10,-50})));
  Annex60.Fluid.Sources.FixedBoundary bou(redeclare package Medium =
        Medium, nPorts=1)
  ...
  Annex60.Fluid.Sources.MassFlowSource_T boundary1(
    ...
    use_m_flow_in=false,
    m_flow=1,
    T=313.15) annotation (Placement(transformation(extent={{0,68},{20,88}})));
...
equation
  ...
  connect(buoyancyModel.port_b, thermalCollector1.port_a) annotation (Line(
      ...
      color={191,0,0},
      smooth=Smooth.None));
  annotation (    __Dymola_Commands(file="modelica://Annex60/Resources/Scripts/Dymola/Fluid/Storage/BaseClasses/BuoyancyModels/Examples/Buoyancy.mos"
        "Simulate and plot"),

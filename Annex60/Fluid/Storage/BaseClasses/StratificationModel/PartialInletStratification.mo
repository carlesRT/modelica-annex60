...
model PartialInletStratification
  ...
  Annex60.Fluid.Sensors.Temperature senTem(redeclare package Medium =
        Medium) annotation (Placement(transformation(extent={{40,44},{20,64}})));
  Annex60.Fluid.Sources.MassFlowSource_T sou[nEle](
    redeclare package Medium = Medium,
    each nPorts=1,
    each m_flow=0,
    each use_m_flow_in=true,
    each use_T_in=true) "Flow source"
